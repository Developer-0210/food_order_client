...
export default function FoodMenuPage() {
  ...
  const callWaiter = async () => {
    if (!tableId) return
    try {
      await axios.post(ROUTES.ORDER.TABLE_REQUEST, { table_id: tableId })
  ...
  const totalItems = cart.reduce((acc, l) => acc + l.qty, 0)
      ...
      <header className="bg-white shadow sticky top-0 z-10">
        <div className="mx-auto max-w-7xl px-4 py-4 flex items-center justify-between">
          ...
          <button
            ...
            className="relative bg-blue-600 hover:bg-blue-700 text-white p-2 rounded-full"
          >
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
          <div className="bg-white rounded-xl shadow-lg w-full max-w-md mx-4 p-6 relative">
            ...
            <button
              ...
              onClick={placeOrder}
              className="w-full mt-3 px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded disabled:opacity-50 hover:bg-blue-700"
            >
              {placing ? "Placing…" : "Place Order"}
            </button>
            {msg && <div className="text-sm text-center text-green-700 mt-2">{msg}</div>}
          </div>
        </div>
      )}
    </div>
